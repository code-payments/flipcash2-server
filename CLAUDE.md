...
- `intent.Integration` - Controls allowed intent types (OpenAccounts, SendPublicPayment, ReceivePaymentsPublicly); denies private payments. `OnSuccess` hooks inject chat messages after DM payments. Also gates the tip that initializes a tip DM on a floor (the recipient's minimum DM chat initialization fee where set, else the `tip/` preset minimum); payments into an initialized chat, tip or send, are any amount.
...
- `e2ee/` - Server side of Signal-protocol end-to-end encrypted messaging (`e2ee.v1` `KeyDistribution` + `Mailbox`), **standalone for now**: DMs only, no chat/event/push integration. `KeyDistributionServer` registers per-device identity keys (registered accounts only, `IsRegistered`, else `DENIED`; unregistering is not gated; the registration also records the device's `capabilities`, served on `Device`/`PreKeyBundle`, required of new devices per `DefaultRequiredCapabilities` (`SPARSE_POST_QUANTUM_RATCHET`, as Signal requires `spqr` of new devices; `WithRequiredCapabilities` replaces the set, none with no args) → `MISSING_REQUIRED_CAPABILITY`, and **extended in place, add-only**, by `SetCapabilities` (`Store.AddCapabilities`, `e2ee.MergeCapabilities`; a roster CAS in DynamoDB, as Signal refreshes capabilities in place rather than forcing a re-registration), and its `app_install`, the push-token scope, never served) certified by the account key (`IdentityKeyMessage`, Ed25519 over `"flipcash-e2ee-identity-key-v1" || key`), verifies every prekey's XEdDSA signature against that device's identity key (`e2ee/xeddsa`: Montgomery→Edwards conversion, sign bit carried in the signature's top bit), hands out prekey bundles that consume one-time keys, and returns a `RepeatedUseDigest` (SHA-256 over identity key, signed-prekey id+key, last-resort id+key, ids as 8-byte big-endian: byte for byte Signal-Server's keys/check digest; `e2ee.RepeatedUseDigest`) on `SetKeys` and `GetPreKeyCounts` so a client can detect a diverged key set. `MailboxServer` relays ciphertext into per-device mailboxes (strictly increasing `Sequence` per mailbox, gaps allowed, held until `AckEnvelopes` or the handler-chosen `ExpiresAt`), refuses a device fetching its own bundle, enforces Sesame's device-mismatch rule (the sender's own user always included: a sender with other devices must name them all or is answered `MISMATCHED_DEVICES` on its own user, Signal's sync-message rule), trusts `client_ts` (only sub-millisecond precision or a value past `maxClientTSAhead`, a year, is `INVALID_TIMESTAMP`; no window behind), does **not** deduplicate a retried send (a recipient absorbs a second copy: libsignal's duplicate detection plus the `MessageId` upsert), and issues `SERVER_DELIVERY_RECEIPT`s on ack, **one per (sender device, chat) per ack call** naming every acknowledged `client_ts` (`DeliveryReceipt.acknowledgements`, shaped like the client `ReceiptMessage`; the chat is the envelope's), so a drain of a thousand messages earns each sender one receipt. A `GetEnvelopes` page is bounded by count (100) **and by content bytes** (`maxGetEnvelopesPageBytes`, 2 MiB, half the gRPC message limit; `WithGetEnvelopesPageBytes` for tests): the store (`GetEnvelopes(..., limit, maxBytes)`) stops after the envelope that crosses the budget, the handler holds that envelope for the next page and answers `has_more`, a lone envelope over the budget is served alone, and a page cut at the mailbox's last envelope is followed by one empty page. Every mailbox call (send, drain, ack) records the calling device's `last_seen` at **day granularity** on the user's own day (`LastSeenDay`: UTC days shifted by `LastSeenOffset`, an FNV hash of the user ID mod 24h, so the fleet's daily writes spread over the day as Signal's do; `Store.TouchLastSeen`: a conditional write only when the stored day is older; the handler's `lastSeenMemo` calls it once per device per day per process, so a fleet costs one write each per day and no read precedes it). The chat is a DM whose ID is `DeriveDmChatID` (domain `flipcash:chat:dm:e2ee`, a stand-in until chat.v1 has an E2EE chat type); group IDs are `NOT_FOUND`, sender-key sends are `InvalidArgument`. **Rate limits** (`e2ee/limit.go`, `Limits`/`DefaultLimits`, Signal-Server's sizes; `WithKeyDistributionLimits`/`WithMailboxLimits`): a `BucketConfig` names a burst (`Size`) and a rate (one per `Refill`) but stores keep it as a **sliding window** of `Size` permits over `Size×Refill` (`Window`, epoch-aligned), the previous window counted at the share still in view (`WindowCounts`, `BucketConfig.Decide`: the same burst and rate as the bucket, exact `retry_after`, a cost above `Size` never served); `Store.TakeTokens(kind, key, cost, cfg, now)` takes the caller's clock so tests are exact. In DynamoDB a take is **one unconditional `ADD`** on the current window's count (conditioned only on the item being on that window, `ReturnValues ALL_NEW`), judged on what comes back, with a refused take `ADD`ing its cost back best effort; only the roll to a new window, once per key per window, reads and compare-and-sets, so a key many callers hit at once never contends (Signal keeps exact buckets in one Redis script; a read-then-CAS lost races under exactly the load a limit exists for). Keyed per (caller device, target device, target registration id) and per caller device for bundles (checked before any prekey is spent, fail closed), and per (sender user, peer user) plus inbound bytes per peer for sends (checked after every other check and before the write, peer only, fail open); both answer `RATE_LIMITED` + `retry_after`. Not built: live delivery (an `online` send is stored ephemeral for `ephemeralWindow`, 10 s, flagged `Ephemeral`, and reaches only a device that drains within it), pushes, device expiry, chat rules. Stores: `memory/` and `dynamodb/` (two tables, no GSI, every read strongly consistent, TTL on `expires_at` on both; layout documented in `e2ee/dynamodb/table.go`; limits are `limit#<kind>#<key>` partitions in the devices table holding `cur_w/cur_n/prev_w/prev_n`, TTL'd two windows out): a user's whole device roster is **one item** CAS-updated per map entry, so lowest-free-id, the 5-device cap, duplicate-identity-key and idempotency checks are one compare-and-set; a device's keys and one-time pools are keyed by a per-registration **nonce** (a reinstall under a reused id never sees its predecessor's keys or mailbox), pools are replaced by writing under a fresh random generation then flipping the keys item (a full pool plus the flip exceeds a transaction; pools are capped at 100 keys per kind, Signal's batch size, so one upload stays inside a partition-second, and the batch loops back off with jitter on unprocessed leftovers and give up after `maxBatchRounds`, tested against a fake endpoint in `batch_test.go`), and pool items are keyed by **upload index** with the prekey id as an attribute (Signal-Server's paged-pool design): the keys item holds per pool `(gen, idx, n)`, a take is one unconditional `ADD idx` conditioned on `idx < n` (`ReturnValues ALL_OLD` names the generation and index reserved, and carries the signed/last-resort keys so a non-empty take never reads the keys item separately) then a `GetItem` at that index, the two pools taken side by side; the write is the arbiter, so takes never contend and never charge a replacement's pool for a retired key, and a read that misses (a replacement flipped **and swept** between reservation and read) reserves again on the new generation (`afterTakeReserve` hook + `race_test.go`); served keys stay in place until their generation is swept; a replacement sweeps only the generation it replaced and discards its own staged pools on refusal, so two concurrent replacements never delete each other's keys; duplicate ids are refused before any write, KEM ids across last-resort + pool and one-time curve ids within their pool (`e2ee.HasDuplicateKemID` / `HasDuplicateOneTimeID`, shared by the handler and both stores; a new last-resort id is checked against the stored pool by a filtered Query over it, served keys included); pool items are stamped `written_at` and every flip sweeps the generation it replaced plus any other generation under the nonce older than `orphanGrace` (1h; Signal's orphaned-page rule, so a replacement that failed or died between staging and flip is collected by the next one, and a generation being staged concurrently is not); an unregistration sweeps the device's keys and mailbox **detached** from the call under `sweepTimeout` (`afterSweep` hook), and pool items carry no TTL (a live pool must never expire under a device), so a registration that never reaches the roster or an unregistration whose sweep fails still leaks its pools until a sweeper exists; mailboxes are one partition per registration whose `#counter` (`seq`, `count`, `bytes`) is moved only by unconditional `ADD`: a delivery reserves its positions first, one ADD per mailbox in parallel, so fan-in never contends, then writes the envelopes with `BatchWriteItem`, 25 per call and the calls side by side (`deliverConcurrency`; a DM is one call, a full group ten), plain unconditional writes with **no atomicity across envelopes** (Signal-Server inserts per device queue the same way; a call that fails partway may have stored some envelopes, and the client's retry stores the whole call again, which recipients absorb), and a delivery that fails after reserving leaves gaps; an ack subtracts `count`/`bytes` best effort in one write per call, so the counters bound a mailbox from above (nothing enforces a cap yet); envelope ids embed the sequence (`e2ee.NewEnvelopeID`, so an ack is a keyed conditional delete returning the old item); there are no dedupe markers; TTL is the envelope's `ExpiresAt` (`envelopeRetention` 30 d or `ephemeralWindow`), and reads filter lapsed items because TTL deletion is lazy. Delivery receipts are still issued from the ack call, not the ack transaction.
- `event/` - Real-time event streaming with bidirectional gRPC streams. See below.
- `push/` - Push notification management (FCM for iOS/Android), with category/group-key support; chunked batch sends; sets the app badge from `badge/`. Chat message pushes are a `ChatMessagePush` built once per message (`BuildContactDmPush` / `BuildTipDmPush` / `BuildGroupChatPush`) and sent per recipient page (`Send`)
...
domain/
  ...
  postgres/          # PostgreSQL implementation (production for most domains)
    store.go
  dynamodb/          # DynamoDB implementation (chat, messaging, blob, badge, blocklist, cluster, e2ee)
    store.go
...
**Storage backend split:** Most domains use Postgres. Chat, messaging, blob, badge, blocklist, cluster and e2ee use **DynamoDB** via `aws-sdk-go-v2`. DynamoDB table definitions live in each package's `dynamodb/table.go`, not in Prisma. Several tables use **sparse GSIs** (e.g. `group_members.by_user` / `by_joined_at`): any new item type written into that partition must omit the indexed attributes or it leaks into the index.
...
Chat, messaging, blob, badge, blocklist, cluster and e2ee data live in **DynamoDB**, not Postgres — table definitions are in the respective `dynamodb/table.go` files.
