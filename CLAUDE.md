...
- `messaging/` - Message persistence, delivery/read pointers, typing notifications (DynamoDB-backed); `sender.go` is the engine for server-initiated messages (e.g., payment messages injected into DMs). Reads gate on membership alone (`canListen`); sends gate on membership plus the chat's rules (`canSpeak`). Membership is meant to track listener rules (a member who stops satisfying one gets removed) — not built yet, so reads never evaluate rules; `GetChat` gates on membership alone
