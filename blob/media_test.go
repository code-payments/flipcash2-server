...
func TestMedia_ShareIntoChat(t *testing.T) {
	ctx := context.Background()
	store := memory.NewInMemory()
	access := memory.NewInMemoryAccessStore()
	media := blob.NewMedia(store, memory.NewInMemoryStorage(), access)
	...
	t.Run("a ready original owned by the sharer is shared and granted", func(t *testing.T) {
		...
		require.NoError(t, media.ShareIntoChat(ctx, owner, chatID, []*blobpb.BlobId{id}))
	...
	t.Run("empty input is a no-op", func(t *testing.T) {
		require.NoError(t, media.ShareIntoChat(ctx, owner, chatID, nil))
	})
	...
	t.Run("a blob owned by someone else is rejected and grants nothing", func(t *testing.T) {
		...
		err := media.ShareIntoChat(ctx, owner, chatID, []*blobpb.BlobId{mine, theirs})
	...
	t.Run("a non-ready blob is rejected", func(t *testing.T) {
		...
		require.ErrorIs(t, media.ShareIntoChat(ctx, owner, chatID, []*blobpb.BlobId{id}), blob.ErrBlobNotShareable)
	})
	...
	t.Run("a rendition is rejected (only originals are shareable)", func(t *testing.T) {
		...
		require.NoError(t, err)
		require.ErrorIs(t, media.ShareIntoChat(ctx, owner, chatID, []*blobpb.BlobId{rendition}), blob.ErrBlobNotShareable)
	})
	...
	t.Run("an unknown blob is rejected", func(t *testing.T) {
		require.ErrorIs(t, media.ShareIntoChat(ctx, owner, chatID, []*blobpb.BlobId{newBlobID(t)}), blob.ErrBlobNotShareable)
	})
}

func TestMedia_Resolve(t *testing.T) {
	ctx := context.Background()
	store := memory.NewInMemory()
	media := blob.NewMedia(store, memory.NewInMemoryStorage(), memory.NewInMemoryAccessStore())
	...
	meta, err := media.Resolve(ctx, []*blobpb.BlobId{ready, pending, unknown})
	require.NoError(t, err)
	...
	empty, err := media.Resolve(ctx, nil)
	require.NoError(t, err)
	require.Nil(t, empty)
}
