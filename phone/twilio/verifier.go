...
func (v *verifier) SendCode(ctx context.Context, phoneNumber string, appHash *string) (string, *phone.Metadata, error) {
...
func (v *verifier) checkValidPhoneNumber(phoneNumber string) error {
	...
	metadata := getMetadataFromCarrierInfoMap(phoneNumber, *resp.Carrier)

	// This is not guaranteed to be available
	if metadata.Type == phone.TypeUnknown {
		return nil
	}
...
func getMetadataFromCarrierInfoMap(phoneNumber string, carrierInfo map[string]interface{}) *phone.Metadata {
	...
	if phoneType != nil {
		...
		if ok {
			metadata.Type = getPhoneTypeFromString(strValue)
		}
	}
	...
	if mobileCountryCode != nil {
		...
		if ok {
			...
			if err == nil {
				metadata.MobileCountryCode = &intValue
			}
		}
	}
	...
	if mobileNetworkCode != nil {
		...
		if ok {
			intValue, err := strconv.Atoi(strValue)
			if err == nil {
				metadata.MobileNetworkCode = &intValue
			}
		}
	}
